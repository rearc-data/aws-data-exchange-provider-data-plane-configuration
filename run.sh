...
STACK_NAME='adx-provider-data-plane' #  name of the cloudformation stack
...
ENV_NAME="prod" # change this if given a custom envname or connecting to testing environments
REGION="us-east-1"  # replace this with the name of the AWS region you are running this cloudformation stack in
...
aws cloudformation deploy --stack-name "$STACK_NAME" \
  ...
  --parameter-overrides ControlPlaneAccount="$CONTROL_PLANE_AWS_ACCOUNT_NUMBER" \
              ...
              OnboardingTopicARN="$ONBOARDING_TOPIC_ARN" \
              EnvName="$ENV_NAME" \
